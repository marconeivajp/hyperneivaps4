...
void executarAcaoMenuVideo(int id) {
    ...
    if (id == 802) aumentarVolume();
    if (id == 803) diminuirVolume();
...
void desenharLinhaDoTempo(uint32_t* tela) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (thumb_ready[i]) {
            for (int y = 0; y < 80; y++) {
                int src_y = y * 108 / 80;
                for (int x = 0; x < 192; x++) {
                    tela[(start_y + y) * 1920 + (thumb_x + x)] = thumb_cache[i][src_y * 192 + x];
                }
            }
        } else {
            for (int y = 0; y < 80; y++) {
                for (int x = 0; x < 192; x++) {
                    if (x < 5 || x > 187 || y < 5 || y > 75) 
                        tela[(start_y + y) * 1920 + (thumb_x + x)] = 0xFF111111;
                    else
                        tela[(start_y + y) * 1920 + (thumb_x + x)] = 0xFF000000;
                }
            }
        }
    }
    
    int audio_y = start_y + 80;
    for(int x = 0; x < 1920; x += 3) {
        int altura_onda = ((x * 17) % 40) + ((x * 7) % 20); 
        if(altura_onda < 4) altura_onda = 4;
        if(altura_onda > 56) altura_onda = 56;
        
        int centro = audio_y + 30;
        for(int y = centro - altura_onda/2; y < centro + altura_onda/2; y++) {
            tela[y * 1920 + x] = 0xFF33CC33;     
            tela[y * 1920 + x + 1] = 0xFF33CC33; 
        }
    }
    
    int playhead_x = (int)((video_time_current / video_time_total) * 1920.0);
    if(playhead_x < 0) playhead_x = 0;
    if(playhead_x > 1918) playhead_x = 1918;
    
    for (int y = start_y - 20; y < 1080; y++) {
        tela[y * 1920 + playhead_x] = 0xFF0000FF; 
        tela[y * 1920 + playhead_x + 1] = 0xFF0000FF;
    }
    ...
    for(int y = 0; y < 15; y++) {
        for(int x = -y; x <= y; x++) {
            int px = playhead_x + x;
            if(px >= 0 && px < 1920) {
                tela[(start_y - 20 + y) * 1920 + px] = 0xFF0000FF;
            }
        }
    }
    
    char t_buf[64];
    sprintf(t_buf, "Tempo: %.2fs / %.2fs", video_time_current, video_time_total);
...
void iniciarVideoMP4(const char* caminho) {
    ...
    // INICIA O RELÓGIO MESTRE DO ZERO
    last_sys_time = sceKernelGetProcessTime();
    playback_time = 0.0;
    video_time_current = 0.0;
}
...
void atualizarVideoFFmpeg(uint32_t* tela) {
    ...
    // ========================================================
    // O RELÓGIO MESTRE E SINCRONIZAÇÃO
    // ========================================================
    ...
    if (delta_sec > 0.1) delta_sec = 0.1; // Limite de 100ms para evitar saltos bruscos
    ...
    // Soft Sync: Mantém o relógio atrelado ao áudio
    if (audioStream != -1 && audio_time_current > 0.1) {
        double diff = playback_time - audio_time_current;
        if (diff > 0.1 || diff < -0.1) {
            playback_time = (playback_time * 0.95) + (audio_time_current * 0.05);
        }
    }

    // 1. O FREIO ABS DO TIKTOK: Se o vídeo estiver adiantado, desenha o antigo e sai (espera o tempo passar)
    if (video_time_current > playback_time) {
        if (pFrameRGB->data[0]) desenharVideoRedimensionadoFast(tela, pFrameRGB->data[0], pCodecCtx->width, pCodecCtx->height, dw, dh, dx, dy);
        desenharLinhaDoTempo(tela);
        return; 
    }
    ...
    static int frames_skipped_consec = 0;
    uint64_t loop_start = sceKernelGetProcessTime();
    int read_ret = 0;

    // 2. O ACELERADOR DE DECODIFICAÇÃO (Para os vídeos pesados)
    // Fica a ler pacotes até encontrar um vídeo que mereça ser desenhado!
    while (!frame_desenhado && (read_ret = av_read_frame(pFormatCtx, packet)) >= 0) {
        if (packet->stream_index == videoStream) {
            if (avcodec_send_packet(pCodecCtx, packet) == 0) {
                while (avcodec_receive_frame(pCodecCtx, pFrame) == 0) {
                    ...
                    // Acelerador: Se estiver atrasado, pula o desenho (poupa GPU) mas decodifica (para avançar no tempo)
                    if (video_time_current < playback_time - 0.05 && frames_skipped_consec < 3) {
                        frames_skipped_consec++;
                        // Ignora o sws_scale para recuperar tempo!
                    } else {
                        frames_skipped_consec = 0;
                        sws_scale(sws_ctx, pFrame->data, pFrame->linesize, 0, pCodecCtx->height, pFrameRGB->data, pFrameRGB->linesize);
                        frame_desenhado = true;
                        
                        // Atualiza a foto da Timeline
                        if (mostrar_timeline && video_time_total > 0) {
                            int thumb_idx = (int)((video_time_current / video_time_total) * 10.0);
                            if (thumb_idx >= 0 && thumb_idx < 10 && !thumb_ready[thumb_idx]) {
                                uint32_t* src = (uint32_t*)pFrameRGB->data[0];
                                int sw = pCodecCtx->width;
                                int sh = pCodecCtx->height;
                                for (int ty = 0; ty < 108; ty++) {
                                    for (int tx = 0; tx < 192; tx++) {
                                        int sx = tx * sw / 192;
                                        int sy = ty * sh / 108;
                                        thumb_cache[thumb_idx][ty * 192 + tx] = src[sy * sw + sx];
                                    }
                                }
                                thumb_ready[thumb_idx] = true;
                            }
                        }
                    }
                }
        ...
        // Trava de Segurança: Não passa mais de 25ms preso neste loop para o PS4 não dar Crash (Lag Spikes)
        if (sceKernelGetProcessTime() - loop_start > 25000) {
            break;
        }
    }
    ...
    // Repetir o vídeo ao chegar ao fim
    if (read_ret < 0) {
...
void pularParaTempo(double tempo) {
    ...
    if (tempo < 0) tempo = 0;
    if (tempo > video_time_total) tempo = video_time_total;
    ...
    int cur_thumb = (int)((tempo / video_time_total) * 10.0);
    for(int i = cur_thumb; i < 10; i++) thumb_ready[i] = false;
}