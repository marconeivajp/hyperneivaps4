...
void acaoCross_Extra() {
    if (sel == 0) {
        // Limpa os gráficos antes de entrar
...
void renderizarControleTeste(uint32_t* p) {
    ...
    desenharTexto(p, "[ ESQUERDA ]", 35, dX, dY + 90, (buttons & ORBIS_PAD_BUTTON_LEFT) ? 0xFFFFFFFF : 0xFF555555);
    desenharTexto(p, "[ DIREITA ]", 35, dX, dY + 135, (buttons & ORBIS_PAD_BUTTON_RIGHT) ? 0xFFFFFFFF : 0xFF555555);
    ...
    // =======================================================
    // PARTE 2: CENTRO (ANALOGICOS E TOUCHPAD COM LINHA CONTINUA)
    // =======================================================
    ...
    int lCenterX = 350; int lCenterY = 600; int radius = 150;
    ...
    desenharTexto(p, "TOUCHPAD (TESTE DE DESENHO CONTiNUO)", 25, tpX + 30, tpY - 30, 0xFF00AAFF);
    ...
    // =======================================================
    desenharTexto(p, "L2:", 30, 250, 920, 0xFFFFFFFF);
    for (int i = 0; i < 255; i++) {
        uint32_t c = (i < l2) ? 0xFF00AAFF : 0xFF333333;
        for (int y = 0; y < 40; y++) p[(920 + y) * 1920 + (320 + i)] = c;
    }
    sprintf(txt, "%d / 255", l2); desenharTexto(p, txt, 25, 590, 925, 0xFFFFFFFF);

    desenharTexto(p, "R2:", 30, 1150, 920, 0xFFFFFFFF);
    for (int i = 0; i < 255; i++) {
        uint32_t c = (i < r2) ? 0xFF00AAFF : 0xFF333333;
        for (int y = 0; y < 40; y++) p[(920 + y) * 1920 + (1220 + i)] = c;
    }
    sprintf(txt, "%d / 255", r2); desenharTexto(p, txt, 25, 1490, 925, 0xFFFFFFFF);
}