...
extern int mapAcoes[20]; // ATUALIZADO PARA 20
...
extern int sfxLigado, sfxVolume; // PUXANDO VARIÁVEIS DE SOM
...
void acaoCross_Editar() {
    if (menuAtual == MENU_EDITAR) {
        if (sel == 14) { // ERA 13, AGORA É 14 O "RESETAR TUDO"
            ...
            sfxLigado = dSfxLigado; sfxVolume = dSfxVolume; // RESETA O SOM
            salvarConfiguracao();
        }
        ...
        if (acaoReal == 9) {
            ...
            else if (editTarget == 13) { sfxLigado = dSfxLigado; sfxVolume = dSfxVolume; } // RESETA O MENU DE SOM
            salvarConfiguracao();
        }
