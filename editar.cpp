...
struct LayoutConfig {
    ...
    int sfxOn, sfxVol; // NOVAS VARIÁVEIS PARA SALVAR O SOM
};
...
const int dSfxLigado = 1, dSfxVolume = 100; // DEFAULTS DO SOM
...
int sfxLigado = dSfxLigado, sfxVolume = dSfxVolume; // CONTROLE ATUAL DO SOM

bool editMode = false; int editTarget = 0, editType = 0; int mapAcoes[20]; // AUMENTADO PARA 20
...
void preencherMenuEditar() {
    ...
    strcpy(nomes[0], "LISTA"); strcpy(nomes[1], "CAPA"); strcpy(nomes[2], "DISCO"); strcpy(nomes[3], "FUNDO"); strcpy(nomes[4], "BARRA LOAD"); strcpy(nomes[5], "MENU AUDIO"); strcpy(nomes[6], "MENU UPLOAD"); strcpy(nomes[7], "FONTE (TEXTO)"); strcpy(nomes[8], "NOTIFICACOES"); strcpy(nomes[9], "EXPLORAR"); strcpy(nomes[10], "ELEMENTO FIXO"); strcpy(nomes[11], "ELEMENTO CTRL"); strcpy(nomes[12], "PONTEIRO");
    strcpy(nomes[13], "ELEMENTOS SONOROS"); // NOVA OPÇÃO
    strcpy(nomes[14], "RESETAR TUDO"); // DESLOCADO PARA 14
    totalItens = 15; menuAtual = MENU_EDITAR;
}
...
void preencherMenuEditTarget() {
    ...
    else if (editTarget == 13) { // OPÇÕES DO MENU ELEMENTOS SONOROS
...
void processarControlesEdicao(unsigned int buttons) {
    ...
    // CONTROLES DE ÁUDIO SFX (Ligar/Desligar e Volume)
