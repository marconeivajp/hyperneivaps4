...
int sfxLigado = 1, sfxVolume = 30, upBg = 0, upTextNorm = 12, upTextSel = 8;
int numSlotsPadrao = 8;
int corSelecao = 10;
int msgColIndex = 12;
int fontColIndex = 12;
int p1Double = 0;
int listLoop = 0;
int listFocus = 0;
int barTextCol = 12; // Branco
int listStyle = 0, fontAnim = 0, listCurvature = 0, listZoomCentro = 0;
...
void salvarConfiguracao() {
    ...
    cfg.lX = listXV; cfg.lY = listYV; cfg.lW = listW; cfg.lH = listH; cfg.cX = capaX; cfg.cY = capaY; cfg.cW = capaW; cfg.cH = capaH; cfg.dX = discoX; cfg.dY = discoY; cfg.dW = discoW; cfg.dH = discoH; cfg.bX = backX; cfg.bY = backY; cfg.bW = backW; cfg.bH = backH; cfg.brX = barX; cfg.brY = barY; cfg.brW = barW; cfg.brH = barH; cfg.numSlotsPadrao = numSlotsPadrao; cfg.corSelecao = corSelecao; cfg.msgColIndex = msgColIndex; cfg.fontColIndex = fontColIndex; cfg.p1Double = p1Double; cfg.listLoop = listLoop; cfg.listFocus = listFocus; cfg.aX = audioX; cfg.aY = audioY; cfg.aW = audioW; cfg.aH = audioH; cfg.uX = upX; cfg.uY = upY; cfg.uW = upW; cfg.uH = upH; cfg.fT = fontTam; cfg.mX = msgX; cfg.mY = msgY; cfg.mTam = msgTam; cfg.lSpcV = listSpcV; cfg.lOri = listOri; cfg.lBg = listBg; cfg.bBg = barBg; cfg.bFill = barFill; cfg.lMk = listMark; cfg.lHMk = listHoverMark; cfg.lXH = listXH; cfg.lYH = listYH; cfg.lSpcH = listSpcH; cfg.fAl = fontAlign; cfg.fSc = fontScroll;
...
void salvarAnimacaoComNome(const char* nomeArquivo) {
    ...
    cfg.msgColIndex = msgColIndex; cfg.fontColIndex = fontColIndex; cfg.p1Double = p1Double;
    cfg.listLoop = listLoop; cfg.listFocus = listFocus; cfg.barTextCol = barTextCol;
...
void carregarConfiguracao() {
    ...
    elem1On = 0; ctrl1On = 0; pont1On = 0; sfxLigado = 1; sfxVolume = 30; upBg = 0; upTextNorm = 12; upTextSel = 8;
    ...
    if (f) {
        ...
        if (lidos >= 1) {
            ...
            if (lidos >= 196) { elem1X = cfg.e1X; elem1Y = cfg.e1Y; elem1W = cfg.e1W; elem1H = cfg.e1H; elem1On = cfg.e1O; ctrl1X = cfg.c1X; ctrl1Y = cfg.c1Y; ctrl1W = cfg.c1W; ctrl1H = cfg.c1H; ctrl1On = cfg.c1O; pont1X = cfg.p1X; pont1Y = cfg.p1Y; pont1W = cfg.p1W; pont1H = cfg.p1H; pont1On = cfg.p1O; pont1Modo = cfg.p1M; pont1Lado = cfg.p1L; sfxLigado = cfg.sfxOn; sfxVolume = cfg.sfxVol; upBg = cfg.uBg; upTextNorm = cfg.uTn; upTextSel = cfg.uTs; numSlotsPadrao = cfg.numSlotsPadrao; if(numSlotsPadrao <= 0) numSlotsPadrao = 8; corSelecao = cfg.corSelecao; if(corSelecao < 0 || corSelecao > 14) corSelecao = 10; msgColIndex = cfg.msgColIndex; if(msgColIndex < 0 || msgColIndex > 14) msgColIndex = 12; fontColIndex = cfg.fontColIndex; if(fontColIndex < 0 || fontColIndex > 14) fontColIndex = 12; p1Double = cfg.p1Double; listLoop = cfg.listLoop; listFocus = cfg.listFocus; }
            if (lidos >= 200) { barTextCol = cfg.barTextCol; if(barTextCol < 0 || barTextCol > 14) barTextCol = 12; }
...
void preencherMenuEditTarget() {
    ...
    if (editTarget == 16) {
        ...
        strcpy(nomes[totalItens], "COR DA FONTE (MENU)"); mapAcoes[totalItens++] = 81;
        ...
        if (editTarget == 13) { strcpy(nomes[totalItens], "MODO ACOMPANHAR/ESTATICO"); mapAcoes[totalItens++] = 13; strcpy(nomes[totalItens], "LADO (ESQ/DIR/CIMA/BAIXO)"); mapAcoes[totalItens++] = 14; 
            strcpy(nomes[totalItens], "PONTEIROS (DUPLO)"); mapAcoes[totalItens++] = 82; 
        }
    }
        ...
        strcpy(nomes[totalItens], "CONFIGURAR BOTOES (COMBOS)"); mapAcoes[totalItens++] = 200;
    }
    else if (editTarget == 19) {
        strcpy(nomes[totalItens], "+ ADICIONAR NOVO COMBO"); mapAcoes[totalItens++] = 201;
        extern SfxCombo combosSFX[20];
        extern const char* getSfxButtonName(uint32_t);
        for (int i = 0; i < 20; i++) {
            if (combosSFX[i].active) {
                sprintf(nomes[totalItens], "COMBO %d: [%s]", i + 1, getSfxButtonName(combosSFX[i].buttons));
                mapAcoes[totalItens++] = 202 + i;
            }
        }
    }
    else if (editTarget == 22) {
        extern int interfaceComboAlvo;
        extern SfxCombo combosSFX[20];
        extern const char* getSfxButtonName(uint32_t);
        sprintf(nomes[totalItens], "1. DEFINIR BOTOES: [%s]", getSfxButtonName(combosSFX[interfaceComboAlvo].buttons)); mapAcoes[totalItens++] = 221;
        sprintf(nomes[totalItens], "2. ESCOLHER SOM: [%s]", (strlen(combosSFX[interfaceComboAlvo].path) > 0) ? strrchr(combosSFX[interfaceComboAlvo].path, '/') + 1 : "VAZIO"); mapAcoes[totalItens++] = 222;
        strcpy(nomes[totalItens], "3. SALVAR COMBO"); mapAcoes[totalItens++] = 223;
        strcpy(nomes[totalItens], "X. DELETAR COMBO"); mapAcoes[totalItens++] = 224;
    }
        ...
        if (editTarget == 0) { // LISTA
            ...
            strcpy(nomes[totalItens], "SLOTS VISIVEIS (LISTA)"); mapAcoes[totalItens++] = 49;
            strcpy(nomes[totalItens], "COR SELECAO"); mapAcoes[totalItens++] = 35;
            strcpy(nomes[totalItens], "LISTA CICLICA (LOOP)"); mapAcoes[totalItens++] = 83;
            strcpy(nomes[totalItens], "FOCO (NORMAL/CENTRAL.)"); mapAcoes[totalItens++] = 84;
            }
            ...
            strcpy(nomes[totalItens], "POSICAO"); mapAcoes[totalItens++] = 0;
            if (editTarget == 9) { strcpy(nomes[totalItens], "TAMANHO DA FONTE"); mapAcoes[totalItens++] = 1; strcpy(nomes[totalItens], "COR DA NOTIFICACAO"); mapAcoes[totalItens++] = 80; }
            else {
            strcpy(nomes[totalItens], "TAMANHO"); mapAcoes[totalItens++] = 1; 
            if (editTarget == 8) { strcpy(nomes[totalItens], "COR DA FONTE (MENU)"); mapAcoes[totalItens++] = 81; }
            else { strcpy(nomes[totalItens], "ESTICAR"); mapAcoes[totalItens++] = 2; }
        }
            if (editTarget == 4 || editTarget == 5) { strcpy(nomes[totalItens], "COR DE FUNDO"); mapAcoes[totalItens++] = 3; strcpy(nomes[totalItens], "COR PREENCH"); mapAcoes[totalItens++] = 8; if(editTarget == 5) { strcpy(nomes[totalItens], "COR DO TEXTO"); mapAcoes[totalItens++] = 90; } }
