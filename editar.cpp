...
struct LayoutConfig {
    ...
    int sfxOn, sfxVol;
    int uBg, uTn, uTs; // NOVAS CORES DO EXPLORAR (Opcoes)
};
...
const int dSfxLigado = 1, dSfxVolume = 100;
const int dUpBg = 0, dUpTextNorm = 12, dUpTextSel = 8; // Padrão: Bg escuro, Texto Normal Branco, Texto Marcado Amarelo Claro
...
int sfxLigado = dSfxLigado, sfxVolume = dSfxVolume;
int upBg = dUpBg, upTextNorm = dUpTextNorm, upTextSel = dUpTextSel;

bool editMode = false; int editTarget = 0, editType = 0; int mapAcoes[30];
...
void salvarConfiguracao() {
    LayoutConfig cfg = {
        ...
        elem1X, elem1Y, elem1W, elem1H, elem1On, ctrl1X, ctrl1Y, ctrl1W, ctrl1H, ctrl1On, pont1X, pont1Y, pont1W, pont1H, pont1On, pont1Modo, pont1Lado, sfxLigado, sfxVolume,
        upBg, upTextNorm, upTextSel
    };
...
void carregarConfiguracao() {
    ...
    if (f) {
        elem1On = dElem1On; ctrl1On = dCtrl1On; pont1On = dPont1On; sfxLigado = dSfxLigado; sfxVolume = dSfxVolume;
        ...
        if (lidos >= 1) {
            ...
            if (lidos == sizeof(LayoutConfig)) {
                ...
                upBg = cfg.uBg; upTextNorm = cfg.uTn; upTextSel = cfg.uTs;
            }
        } fclose(f);
    }
}
...
void preencherMenuEditar() {
    ...
    strcpy(nomes[0], "LISTA"); strcpy(nomes[1], "CAPA"); strcpy(nomes[2], "DISCO"); strcpy(nomes[3], "FUNDO"); strcpy(nomes[4], "BARRA LOAD"); strcpy(nomes[5], "MENU AUDIO"); strcpy(nomes[6], "MENU UPLOAD"); strcpy(nomes[7], "FONTE (TEXTO)"); strcpy(nomes[8], "NOTIFICACOES"); strcpy(nomes[9], "EXPLORAR"); strcpy(nomes[10], "ELEMENTO FIXO"); strcpy(nomes[11], "ELEMENTO CTRL"); strcpy(nomes[12], "PONTEIRO"); strcpy(nomes[13], "ELEMENTOS SONOROS"); strcpy(nomes[14], "RESETAR TUDO"); totalItens = 15; menuAtual = MENU_EDITAR;
}
...
void processarControlesEdicao(unsigned int buttons) {
    ...
    if (editType == 3) { if (editTarget == 4) { if (buttons & ORBIS_PAD_BUTTON_LEFT) { barBg--; if (barBg < 0) barBg = 14; } if (buttons & ORBIS_PAD_BUTTON_RIGHT) { barBg++; if (barBg > 14) barBg = 0; } } else { if (buttons & ORBIS_PAD_BUTTON_LEFT) { listBg--; if (listBg < 0) listBg = 14; } if (buttons & ORBIS_PAD_BUTTON_RIGHT) { listBg++; if (listBg > 14) listBg = 0; } } }
        ...
        if (buttons & ORBIS_PAD_BUTTON_LEFT) { sfxVolume -= 5; if (sfxVolume < 0) sfxVolume = 0; }
        if (buttons & ORBIS_PAD_BUTTON_RIGHT) { sfxVolume += 5; if (sfxVolume > 100) sfxVolume = 100; }
    }
    // NOVOS CONTROLOS DO MENU OPÇÕES (EXPLORAR)
    else if (editType == 17) {
        if (buttons & ORBIS_PAD_BUTTON_UP) upY -= 2; if (buttons & ORBIS_PAD_BUTTON_DOWN) upY += 2;
        if (buttons & ORBIS_PAD_BUTTON_LEFT) upX -= 2; if (buttons & ORBIS_PAD_BUTTON_RIGHT) upX += 2;
    }
    else if (editType == 18) {
        if (buttons & ORBIS_PAD_BUTTON_UP) { upH -= 2; upW -= 2; } if (buttons & ORBIS_PAD_BUTTON_DOWN) { upH += 2; upW += 2; }
    }
    else if (editType == 19) {
        if (buttons & ORBIS_PAD_BUTTON_UP) upH -= 2; if (buttons & ORBIS_PAD_BUTTON_DOWN) upH += 2;
        if (buttons & ORBIS_PAD_BUTTON_LEFT) upW -= 2; if (buttons & ORBIS_PAD_BUTTON_RIGHT) upW += 2;
    }
