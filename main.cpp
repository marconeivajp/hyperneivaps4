...
// 2. CORREÇÃO DE INTELLISENSE PARA VISUAL STUDIO (Remove linhas vermelhas de funções internas)
...
// 5. Definições de Fallback para Menus (Necessário para a lógica de renderização)
#ifndef MENU_MUSICAS
...
// --- VARIÁVEIS GLOBAIS DE SISTEMA (Ficam no main para gestão de hardware) ---
int video = 0, bA = 0;
...
// Assets Visuais do Sistema
unsigned char* capasAssets[6], * discosAssets[6], * backImg = NULL;
int wC[6], hC[6], cC[6], wD[6], hD[6], cD[6], wB, hB, cB;

// --- DECLARAÇÕES EXTERNAS (Resolvendo símbolos de outros ficheiros) ---
extern unsigned char* imgPreview;
extern int wP, hP, cP;

// CORREÇÃO: Ajustado para 128 para evitar conflito com a definição em outros ficheiros .cpp
extern char ultimoJogoCarregado[128];

// Variáveis e funções do módulo de Áudio (HUD Visual)
...
int main(void) {
    // Inicialização da infraestrutura de rede e som
    initNetwork();
    inicializarAudio();

    // Carregamento de Módulos do Sistema PS4 (Com Cast para compatibilidade C++)
    sceSysmoduleLoadModuleInternal((OrbisSysModuleInternal)0x0080); // Common Dialog
    sceSysmoduleLoadModule((OrbisSysModule)0x00A2); // IME Dialog
    sceCommonDialogInitialize();

    // Inicialização de Utilizador e Pad (Comando)
    ...
    // Configuração de Double Buffering de Vídeo em Memória Direta
    ...
    // Configuração de Memória Direta para o Teclado (Resolve o Crash da Tecla)
    off_t imePh;
    void* imeVm = NULL;
    ...
    // Escrita do título nativo em UTF-16 para o diálogo
    uint16_t t[] = { 'E','s','c','r','e','v','a',' ','n','a',' ','F','o','l','h','a','\0' };
    memcpy(pImeTitle, t, sizeof(t));

    // Preparação do sistema de ficheiros e carregamento da configuração
    inicializarPastas();
    carregarConfiguracao();

    // Carregamento de Fontes TrueType (TTF)
    int fd = sceKernelOpen("/app0/assets/fonts/font.ttf", 0, 0);
    if (fd >= 0) {
        off_t fs = sceKernelLseek(fd, 0, 2);
        sceKernelLseek(fd, 0, 0);
        size_t mSz = (fs + 0x1FFFFF) & ~0x1FFFFF;
        off_t pFont;
        ...
        sceKernelRead(fd, ttf, fs);
        sceKernelClose(fd);
        temF = stbtt_InitFont(&font, ttf, 0);
    }

    // Carregamento de Imagens (Fundo, Capas e Discos)
    ...
    // Inicializa a lista do menu principal
    preencherRoot();
    ...
    for (;;) {
        ...
        // 1. Limpeza do Framebuffer e Desenho do Fundo
        ...
        // 2. PROCESSAMENTO DE CONTROLES (Delegado ao módulo controle.cpp)
        processarComando(pData.buttons, uId);

        // 3. LÓGICA DO TECLADO VIRTUAL (IME)
        if (tecladoAtivo) {
        ...
        if (menuAtual == MENU_NOTEPAD) {
            // UI do Bloco de Notas (Papel e Barra)
            ...
            // Renderização das Listas de Menus
            for (int i = 0; i < 6; i++) {
                int gIdx = i + off;
                if (gIdx >= totalItens) break;
                int yP = listY + (i * 120);
                ...
                for (int by = 0; by < listH; by++) {
                    for (int bx = 0; bx < listW; bx++) {
                        int px = listX + bx;
                        int py = yP + by;
                        if (px >= 0 && px < 1920 && py >= 0 && py < 1080) pFrame[py * 1920 + px] = corFundo;
                    }
                }
        ...
        // 5. RENDERIZAÇÃO DE ASSETS (Capas, Discos e Scraper)
        ...
        // 6. MENU DE OPÇÕES (Dropdown Contextual)
        if (showOpcoes) {
        ...
        // Mensagem de Status (Toast)
        if (msgTimer > 0) { desenharTexto(pFrame, msgStatus, 40, 100, 950, 0xFFFFFFFF); msgTimer--; }

    SUBMIT_FRAME:
        // Envio do buffer para saída de vídeo e troca de buffers
        sceVideoOutSubmitFlip(video, bA, 1, 0);
        bA = (bA + 1) % 2;
        sceKernelUsleep(16000); // 60 FPS aprox.
    }
}