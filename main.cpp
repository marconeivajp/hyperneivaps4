...
// --- INSTÂNCIA GLOBAL DA TORRENT ENGINE ---
...
void ligarInterfaceGrafica() {
    ...
    if (!backImg) {
        ...
        const char* caminhos[] = {
            "/data/HyperNeiva/configuracao/imagens/0_Defalt_Background.png",
            "/data/HyperNeiva/configuracao/imagens/0_Defalt_Background.jpg",
...
int main(void) {
    ...
    backImg = stbi_load("/data/HyperNeiva/configuracao/imagens/0_Defalt_Background.png", &wB, &hB, &cB, 4);
    if (!backImg) backImg = stbi_load("/data/HyperNeiva/configuracao/imagens/0_Defalt_Background.jpg", &wB, &hB, &cB, 4);
    if (!backImg) backImg = stbi_load("/data/HyperNeiva/configuracao/0_Defalt_Background.png", &wB, &hB, &cB, 4);
    ...
    // --- INICIALIZAÇÃO DA TORRENT ENGINE (Após o Jailbreak) ---
    g_torrentApp.init();
    ...
    for (;;) {
        ...
        // --- ATUALIZADOR DO TORRENT (Executa em background) ---
        g_torrentApp.update();
        ...
        if (scePadReadState(pad, &pData) == 0) {
            ...
            verificarSfxCombos(pData.buttons);
            extern void verificarTecladoVirtual();
    ...
    // --- DESLIGAR A TORRENT ENGINE ---
    g_torrentApp.shutdown();
