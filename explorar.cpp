...
#include "torrent.h" // ADICIONADO PARA SUPORTE A TORRENT
...
void preencherOpcoesContexto(const char* nomeArquivo) {
    ...
    if (nomeArquivo) {
        ...
        if (strstr(temp, ".torrent") != NULL) isTorrent = true;
    }

    if (isZip) { listaOpcoes[totalOpcoes] = "extrair zip / avatar"; mapOpcoes[totalOpcoes] = 7; totalOpcoes++; }
    ...
    if (isTorrent) {
        listaOpcoes[totalOpcoes] = "baixar torrent";
        mapOpcoes[totalOpcoes] = 50;
        totalOpcoes++;
    }
}
...
void acaoArquivo(int idxOpcao) {
    ...
    case 50: { // AÇÃO DO TORRENT
        ...
        sprintf(caminhoArquivo, "%s/%s", pExplorar, nomeReal);
